...
exports.getPossibleWords = void 0;
// Equality util for comparing the equality of an arbitrary number of points.
function pointsAreEqual(...points) {
    const areEqual = points.reduce((bool, point, i) => {
...
function mapPoints(points, by) {
    return points.map((point) => point[by]);
}
...
function getAdjacentPoints(point, points) {
    ...
    for (let w = point.x - 1; w < point.x + 2; w++) {
        for (let h = point.y - 1; h < point.y + 2; h++) {
            const adjacentPoint = points.find((p) => p.x === w && p.y === h);
...
function getPossibleWords(points, dictionary) {
    points = points.filter((point) => point.letter);
    function getPossibleWordsFromPoint(point, possibleWordPoints = [], // The ordered list of points making up a word.
    possibleWordsPoints = [] // A list of possibleWordPoints.
    ) {
        ...
        entryPoints = entryPoints.filter((adjacentPoint) => {
            return !possibleWordPoints.find((wordPoint) => pointsAreEqual(wordPoint, adjacentPoint));
        });
        // console.log("entryPoints", entryPoints)
        const recordExactMatch = (possibleWordPoints) => {
            const match = dictionary.find((word) => word === getWord(possibleWordPoints));
            if (match &&
                // Ensure this words has not already been added to possibleWordsPoints.
                !possibleWordsPoints.find((word) => getWord(word) === getWord(possibleWordPoints))) {
        ...
        if (entryPoints.length) {
            entryPoints.forEach((entryPoint) => {
                ...
                const match = dictionary.find((word) => word.indexOf(getWord(nextPossibleWord)) !== -1);
                if (match) {
                    recordExactMatch(nextPossibleWord);
                    getPossibleWordsFromPoint(entryPoint, nextPossibleWord, possibleWordsPoints);
                }
            });
        }
        else {
    ...
    const possibleWordsPoints = points.reduce((acc, point) => getPossibleWordsFromPoint(point, [], acc), []);
    return possibleWordsPoints.sort((possibleWordPoint1, possibleWordPoint2) => dictionary.indexOf(getWord(possibleWordPoint1)) -
        dictionary.indexOf(getWord(possibleWordPoint2)));
}
exports.getPossibleWords = getPossibleWords;
function createPoints(w = 1, h = 1) {
    const points = [];
    const letters = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"]; // prettier-ignore
